...
if (window === window.top) {
  console.log('[ChatGPT CS] Injected into ChatGPT top-level page');
...
async function fetchImageAsDataUrl(src: string): Promise<string | null> {
  try {
    ...
    console.warn('[ChatGPT CS] Failed to convert image to data URL:', e);
    return null;
  }
}

// Report a completed job. We convert the image to a data URL IN-PAGE because:
//  - blob: URLs only exist in this tab and are unreachable from the background;
//  - ChatGPT's remote image URLs are authenticated, so the background often can't
//    fetch them, but this page already has the credentials/session.
// Falls back to the raw URL if conversion fails (e.g. cross-origin without CORS).
  ...
  return new Promise((resolve, reject) => {
    ...
    const interval = setInterval(() => {
      ...
      if (el) {
        ...
        clearInterval(interval);
        reject(new Error('ChatGPT prompt input area not found'));
      }
    }, 250);
  });
}
...
async function startGenerationFlow(prompt: string, images?: string[]) {
  ...
  try {
    ...
    console.error('[ChatGPT CS] Generation flow failed:', err.message);
...
function injectText(textarea: HTMLElement, text: string) {
  console.log('[ChatGPT CS] Injecting text to textarea...');
  textarea.focus();
  ...
  try {
    ...
    console.warn('[ChatGPT CS] Clipboard paste simulation failed:', e);
  }
  ...
  if (currentText.trim().length < 5) {
    console.log('[ChatGPT CS] Paste simulation did not populate textarea. Falling back to execCommand...');
    try {
      textarea.innerHTML = '';
      ...
      console.warn('[ChatGPT CS] execCommand fallback failed:', e);
    }
  }
  ...
  if (finalCheckText.trim().length < 5) {
    console.log('[ChatGPT CS] execCommand failed to populate textarea. Setting innerText directly...');
  ...
  try {
    ...
    console.warn('[ChatGPT CS] event dispatch failed:', e);
  }
}
  ...
  setTimeout(() => {
    monitorInterval = setInterval(async () => {
      ...
      if (findRateLimitText(initialText, prompt)) {
        console.warn('[ChatGPT CS] Rate limit detected!');
      ...
      if (errorText) {
        ...
        if (checkedCount > 5) {
          console.error('[ChatGPT CS] Page error detected:', errorText);
      ...
      if (isBusy) {
        if (!isGenerating) {
          console.log('[ChatGPT CS] Generation started (busy indicator/disabled send button detected)');
...
async function checkForCompletedImage() {
  const newImgSrc = findNewImage();
  if (newImgSrc) {
    console.log('[ChatGPT CS] Generated image found:', newImgSrc);
  ...
  // ChatGPT image generation can take several minutes (longer when many background
  // worker tabs are throttled), so allow a generous window before giving up.
  ...
  if (elapsed > 480000) { // 8 minutes timeout
    ...
    console.log('[ChatGPT CS] Image not found yet. Retrying in 3s...');
