...
async function startGenerationFlow(prompt: string, images?: string[]) {
  ...
  try {
    ...
    if (images && images.length > 0) {
      await attachReferenceImages(textarea, images, addLog, 'Gemini');
...
// True if the element sits inside a user-authored query. Used to skip the
// reference image the user uploaded (it renders in the user query with a fresh
// URL that isn't in our pre-send snapshot, so it would otherwise look "new").
function isInsideUserTurn(el: HTMLElement): boolean {
  let node: HTMLElement | null = el;
  while (node) {
    const tag = (node.tagName || '').toLowerCase();
    const cls = typeof node.className === 'string' ? node.className.toLowerCase() : '';
    if (tag === 'user-query' || cls.includes('user-query') || cls.includes('query-content') || cls.includes('query-text')) return true;
    if (tag === 'model-response' || cls.includes('model-response')) return false;
